...
const toTitleCase = (arr) => {
  return arr.map(string => (string.replace(/./, string.charAt(0).toUpperCase())));
};
...
let biggerThanLuke = (arr) => {
  let str = '';
  let lukesMass = parseInt(arr[0].mass);
  arr.forEach(character => {
    if (character.mass > lukesMass) {
      str = str.concat(' - ', character.name);
    }
